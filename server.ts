...
async function startServer() {
  ...
  app.post('/api/wordpress-proxy', async (req, res) => {
    ...
    let wpUrl = process.env.VITE_WORDPRESS_API_URL || 'https://sorasuchi.wasmer.app/graphql';
    
    // Normalize and clean
    wpUrl = wpUrl.trim().replace(/\.+$/, ''); 
    
    // Force transition to new host if the environment variable is still pointing to the old one
    if (wpUrl.includes('sorasuchi.ct.ws') || wpUrl.includes('massive-paint.localsite.io')) {
      console.log(`WordPress Proxy: Redirecting from old host ${wpUrl} to new wasmer.app host`);
      wpUrl = 'https://sorasuchi.wasmer.app/graphql';
    }
    ...
    const tryFetch = async (url: string, silent: boolean = false): Promise<Response | null> => {
      try {
        const h: Record<string, string> = getBaseHeaders();
        if (authUser && authPass) {
          h['Authorization'] = `Basic ${Buffer.from(`${authUser}:${authPass}`).toString('base64')}`;
        }

        if (!silent) console.log(`WordPress Proxy: Connection attempt to ${url}...`);
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), silent ? 5000 : 12000); 
        ...
        if (!response.ok || !isJson) {
          ...
          attempts.push({ 
            ...
            method: 'POST', 
            contentType,
            error: `${statusPrefix}. Body: ${errorText.substring(0, 100).replace(/\s+/g, ' ')}...` 
          });
          
          if (!silent) console.warn(`WordPress Proxy: POST ${url} failed validation (${statusPrefix}, ${contentType})`);
          ...
          if (!isJson || response.status === 405 || response.status === 404 || response.status === 503 || response.status === 403) {
            try {
              ...
              attempts.push({ 
                ...
                error: `Non-JSON response. Body: ${getText.substring(0, 100).replace(/\s+/g, ' ')}...` 
              });
            } catch (err: any) {
